...
- (void) setSquareMode:(CDVInvokedUrlCommand*)command{
    ...
    {
        self.enabledSquareMode = (BOOL)[command.arguments[0] boolValue];
    }
...
- (void) invokeTakePicture:(CGFloat) maxWidth withHeight:(CGFloat) maxHeight {
    ...
    [self.sessionManager.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef sampleBuffer, NSError *error) {
        ...
        if (error) {
            ...
            CGRect imageRect = CGRectMake(finalCImage.extent.origin.x , finalCImage.extent.origin.y ,
                                          finalCImage.extent.size.width, finalCImage.extent.size.height);
            
            if(self.enabledSquareMode){
                
                CGSize size = finalCImage.extent.size;
                
                CGFloat offset = 0;
                
                if(size.height > size.width){
                    offset = (size.height - size.width) / 2;
                    imageRect.size.height = size.width;
            ...
            CGImageRef finalImage = [self.cameraRenderController.ciContext createCGImage:finalCImage fromRect:imageRect];
